...
// ? this function reads the Dates API from website and prints the relevant information
func handleDates(w http.ResponseWriter, r *http.Request) {
	dates, err := getDates("https://groupietrackers.herokuapp.com/api/dates")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	//* Executes the template by passing the Dates' data
...
func getDates(filePath string) ([]Date, error) {
	...
	//* Decoding JSON content in an Dates slice
	var dates []Date
